 ...
 * 
 * 参考：https://blog.csdn.net/miaoch/article/details/53172743
 * 
 * @author jx003
 *
 */
public class CustomEmailService {
	...
	/**
	 * 发送邮件
	 * 接收人支持所有邮件地址，只是发送人邮箱只支持qq，如果需要更改，需要修改服务器及端口等信息
	 * <br/>所有格式都用英文逗号","分隔
	 * 
	 * @param to		(必填)接收人，收件人 可以发给其他邮箱(163等) 下同
	 * 					<br/>支持批量 格式：new String[] {"xxx@qq.com","xxx@qq.com"}
	 * @param cc		(可选)抄送，支持批量 格式：new String[] {"xxx@qq.com","xxx@qq.com"}
	 * @param bcc		(可选)密送，支持批量 格式：new String[] {"xxx@qq.com","xxx@qq.com"}
	 * @param fileList	(可选)附件，发送附件列表 可以写绝对路径 也可以写相对路径(起点是项目根目录)
	 * 					<br/>支持批量 格式：new String[] {"file\\附件1.txt","file\\附件2.txt"}<br/>支持附件重命名(可选) 格式：new String[] {"file\\附件1.txt,text1.txt","file\\附件2.txt,text2.txt"}
	 * @param subject	(必填)主题
	...
	private void addTach(String[] fileList, Multipart multipart) throws MessagingException, UnsupportedEncodingException {
		for (int index = 0; index < fileList.length; index++) {
			...
			mailArchieve.setFileName(MimeUtility.encodeText(displayname, DefaultSettings.get("CustomMailService.encode"), "B"));
